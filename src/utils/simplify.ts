 ...
 */
export function douglasPeucker(points: Point[], epsilon: number, minPoints = 3): Point[] {
    ...
    if (maxDistance > epsilon) {
        ...
        return [start, end].filter((p): p is Point => p !== undefined)
    }
}
...
function preserveMinimumPoints(points: Point[], minPoints: number): Point[] {
...
export function simplifyStroke(points: Point[], strokeWidth: number, minPoints = 3): Point[] {
    if (!points || points.length <= minPoints) {
        return points
    }
